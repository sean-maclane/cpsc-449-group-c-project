...
def create_post():
    ...
    if _post_title is not None and _post_community is not None and _post_body is not None:
        db.execute("INSERT INTO posts (community, title, text, Username, dt, upvotes, downvotes) VALUES (?, ?, ?, ?, ?, ?, ?)", (_post_community, _post_title, _post_body, _username, datetime.now(), 0, 0))
        db.commit()
